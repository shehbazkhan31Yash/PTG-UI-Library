...
export const PtgUiTable: React.FC<TableProps> = ({
    ...
    const handleSort = (accessor: string) => {
        setSortConfig((prevConfig) => {
            const isAscending = prevConfig.key === accessor && prevConfig.direction === 'ascending';
            return { key: accessor, direction: isAscending ? 'descending' : 'ascending' };
        });
    };
            ...
            <div className="search-bar-container">
                <div className="input-container">
                    <input
                        ...
                        className="ptg-ui-input"
                    />
                    {filterValue && (
                        <button onClick={resetFilter} className="reset-button" aria-label="Reset filter">
                            &times;
                        </button>
                    )}
                </div>
                ...
                <table className="ptg-ui-table">
                    <thead className={stickyHeader ? 'sticky-header' : ''}>
                        <tr>
                            {columns?.map((column) => {
                                const sortIndicator =
                                    hoveredColumn === column.accessor && sortConfig.key === column.accessor
                                ...
                                return (
                                    <th
                                        key={column.accessor}
                                        style={{ width: column.columnWidth ?? 'auto' }}
                        ...
                        {sortedData.length ? (
                            sortedData.map((row, rowIndex) => (
                                <tr
                                    ...
                                    className={alternateRowColor && rowIndex % 2 === 0 ? 'alternate-row' : ''}
                                >
                                    {columns?.map((column) => (
                                        <td
                                            key={column.accessor}
