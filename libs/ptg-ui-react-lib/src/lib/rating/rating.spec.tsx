...
describe('Rating Component', () => {
	...
	it('renders the correct number of stars', () => {
		const { container } = render(<PtgUiRating onChange={mockOnChange} />);
		const stars = container.querySelectorAll('.rating-star');
		expect(stars.length).toBe(5);
	});

	it('renders filled stars based on the value prop', () => {
		const { container } = render(<PtgUiRating value={3} onChange={mockOnChange} />);
	...
	it('does not call onChange when readOnly is true', () => {
		const { container } = render(<PtgUiRating readOnly value={3} onChange={mockOnChange} />);
		const stars = container.querySelectorAll('.rating-star');
	...
	it('does not call onChange when disabled is true', () => {
		const { container } = render(<PtgUiRating disabled value={3} onChange={mockOnChange} />);
		const stars = container.querySelectorAll('.rating-star');
		fireEvent.click(stars[2]);
		expect(mockOnChange).not.toHaveBeenCalled();
	});
	...
	it('renders custom icons for filled and empty stars', () => {
		const { container } = render(<PtgUiRating icon="🔥" emptyIcon="⚪" onChange={mockOnChange} />);
		const filledStars = container.querySelectorAll('.star-fill');
	...
	it('renders custom colors for filled and empty stars', () => {
		const { container } = render(<PtgUiRating color="red" borderColor="blue" onChange={mockOnChange} />);
		const filledStars = container.querySelectorAll('.star-fill');
		const emptyStars = container.querySelectorAll('.star-empty');
		filledStars.forEach((star) => {
