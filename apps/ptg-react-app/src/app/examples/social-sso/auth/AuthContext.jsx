...
export const AuthProvider = ({ children }) => {
  ...
  const loginWithSocial = async (socialUser) => {
    try {
      const res = await axios.get("http://localhost:1337/api/user-check/email", {
        params: {
  ...
  const completeSignup = async (formData) => {
    try {
      const res = await axios.post("http://localhost:1337/api/cutom-user-table/create-custom-user", {
