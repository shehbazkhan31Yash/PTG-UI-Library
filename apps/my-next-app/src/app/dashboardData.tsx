export const dashboardCardData = [
  {
    title: 'PTG Angular',
    description: 'This application demonstrate reusable Angular components.',
    view: 'View',
    viewUrl: '/ptg-angular-app',
    documentationUrl: '/ptg-angular-storybook',
    documentation: 'Documentation',
    download: '',
  },
  {
    title: 'PTG React',
    description: 'This application demonstrate reusable React components.',
    view: 'View',
    viewUrl: '/ptg-react-app',
    documentationUrl: '/ptg-react-storybook',
    documentation: 'Documentation',
    download: '',
  },
  {
    title: 'PTG Interview Assessment Portal',
    description: 'Interview Assessment portal to assess and track candidates',
    view: 'View',
    viewUrl:
      'https://yash-ui-apps.azurewebsites.net/ptg-interview-assessment-portal',
    documentation: '',
    download: '',
  },
  {
    title: 'PTG Document Assessment Portal',
    description:
      'Document Assessment portal to assess and track candidates document',
    view: 'View',
    viewUrl: '/ptg-angular-doc-process',
    documentation: '',
    download: '',
  },
  {
    title: 'PTG Google Map',
    description:
      'Provides detailed information about geographical regions and sites worldwide.',
    view: 'View',
    viewUrl: '/ptg-angular-google-map',
    documentation: '',
    download: '',
  },
  {
    title: 'PTG Animations',
    description:
      'The animation is a technique in which images are manipulated to appear as moving images',
    view: 'View',
    viewUrl: '/ptg-react-animations',
    documentation: '',
    download: '',
  },
  {
    title: 'PTG UI Schematics',
    description:
      'This tool is used to generate UI apps for modern frameworks like Angular,React with best practices needed for enterprises apps.',
    view: '',
    documentation: '',
    download: 'Download',
  },
  {
    title: 'PTG React Web Components',
    description:
      'This application demonstrates how to consume stencil web components in a React application.',
    view: 'View',
    viewUrl: '/ptg-ui-web-comp-react',
    documentation: '',
    download: '',
  },
  {
    title: 'PTG Angular Web Components',
    description:
      'This applications demonstrate how to consume stencil web components in a Angular application.',
    view: 'View',
    viewUrl: '/ptg-ui-web-comp-angular',
    documentation: '',
    download: '',
  },
  {
    title: 'PTG Metaverse Components',
    description:
      'This applications demonstrates how to consume stencil web components in a React application.',
    view: 'View',
    viewUrl: '/ptg-metaverse-comp-react',
    documentation: '',
    download: '',
  },
  {
    title: 'PTG Best Practices Documents',
    description: 'Portal to access best practices documents',
    view: 'View',
    viewUrl:
      'https://yash-ui-apps.azurewebsites.net/ptg-best-practices-documents',
    documentation: '',
    download: '',
  },
  {
    title: 'PTG Courses Link',
    description: 'Portal to access different Courses Links',
    view: 'View',
    viewUrl:
      'https://yashtechn.plateau.com/learning/user/personal/landOnPortalHome.do?OWASP_CSRFTOKEN=Q4IE-FKDI-O2Q0-48US-38KJ-8J6D-9ADI-181X&fromSF=Y&fromDeepLink=true&pageID=',
    documentation: '',
    download: '',
  },
];
