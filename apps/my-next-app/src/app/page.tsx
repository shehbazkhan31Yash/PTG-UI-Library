...
export default function Home() {
  return (
    ...
    <div className='text-center pb-4'>
      <Title level={1} className='mb-4' style={{ marginBottom: '0.5rem',color:'#0056b3'}}>PTG UI Apps</Title>
      <p>Below is the list of UI competencies apps.</p>
    </div>
    <Flex wrap gap="large" justify='center'>
        {dashboardCardData.map((it, index) => {
          console.log(it.view, it.viewUrl);
          return (
            <Card key={index + 1} style={{ width: 370}} className='dashboardCard'>
              <Card.Meta
                title={<Title level={4} className='dashboardCardTitle'>{it.title}</Title>} 
                description={<><p className='pb-4 text-black'>
                  {it.description}
                </p></>} />
...
<Button color='geekblue' style={{ background: "#0056b3",color:'#fff' }} size='large' variant="solid">
  {it.download}
</Button>}
 </Space>
  </Card>
